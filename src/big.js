...
export default class InputNumber extends React.Component {
  ...
  fixed = (num, precision) => {
    ...
    if (!this.props.focused) {
      return otherNum.round(precision, bigDecimal.RoundingModes.DOWN).value;
    }
    ...
    let precisionFixed = precision;
    if (des.indexOf('.') > 0) {
      const desLength = des.split('.')[0].length;
      precisionFixed = 14 - desLength < precision ? 14 - desLength : precision;
      return Number(num).toFixed(precisionFixed);
    }
    precisionFixed = 14 - des.length < precision ? 14 - des.length : precision;
    return otherNum.round(precisionFixed, bigDecimal.RoundingModes.DOWN).value;
  };
  ...
  getValidValue(value, min = this.props.min, max = this.props.max) {
    ...
    const bigVal = new bigDecimal(value).value;
    ...
    return new bigDecimal(val).value;
  }

  setValue(v, callback) {
    // trigger onChange
    const { precision } = this.props;
    const num = new bigDecimal(v).value;

    const newValue = this.isNotCompleteNumber(parseFloat(v, 10)) ? null : num;
    const { value = null, inputValue = null } = this.state;
    // https://github.com/ant-design/ant-design/issues/7363
    // https://github.com/ant-design/ant-design/issues/16622
    const newValueInString =
      typeof newValue === 'number' ? this.fixed(newValue, precision) : `${newValue}`;
  ...
  toPrecisionAsStep(num) {
    ...
    if (!isNaN(precision)) {
      return this.fixed(num, precision);
    }
    return num.toString();
  }
  ...
  toNumber(num) {
    ...
    if (isValidProps(this.props.precision)) {
      return this.fixed(num, this.props.precision);
    }
    return num;
  }
  ...
  render() {
    const props = { ...this.props };
    const { prefixCls, disabled, readOnly, useTouch, autoComplete, upHandler, downHandler } = props;
